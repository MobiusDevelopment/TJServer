...
public class UpdatePlayerMainInfoHandler implements PacketHandler
{
	...
	public Packet processing(Packet packet)
	{
		...
		return new Packet(PacketType.RESPONSE, player.getLevel(), player.getPlayerClass().name(), player.getSex() == Sex.MALE ? "�?уж." : "Жен.", player.getExp(), player.getCurrentHp(), player.getCurrentMp());
	}
}

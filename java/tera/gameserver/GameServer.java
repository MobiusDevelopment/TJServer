...
public class GameServer extends ServerThread implements DeadLockListener
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		final ConnectFactory connoctFactory = dataBaseManager.getConnectFactory();
		final Connection con = connoctFactory.getConnection();
		final Statement stmt = con.createStatement();
		final ResultSet rset = stmt.executeQuery("SELECT objectId FROM `character_faces`");
		
		while (rset.next())
		{
			final int objectId = rset.getInt(1);
			final DeprecatedPlayerFace deprFace = dataBaseManager.restoreFace(objectId);
			...
			final PlayerAppearance appearance = deprFace.toAppearance();
	 ...
	 * @see rlib.DeadLockListener#onDetected(ThreadInfo)
	 */
	@Override
